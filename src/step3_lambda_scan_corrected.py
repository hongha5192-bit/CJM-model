#!/usr/bin/env python
"""
Step 3: Lambda scan with 15 points using corrected simulations
Tests lambdas from 1e-2 to 1e5 using 512 corrected simulations
Optimized for Mac Air M4
"""
import numpy as np
import pandas as pd
import json
...
def main():
    """Run lambda scan with 15 points optimized for Mac Air M4 using corrected simulations"""

    logger.info("="*80)
    logger.info("LAMBDA SCAN WITH 15 POINTS - CORRECTED SIMULATIONS")
    logger.info("="*80)
    ...
    logger.info(f"Found {len(sim_files)} corrected simulations")
    ...
    csv_path = output_dir / 'lambda_scores_corrected.csv'
    df_results['K'] = K
    ...
    json_path = output_dir / 'best_lambda_corrected.json'
...
def create_bac_plot(df_results, best_lambda, best_bac, K, output_dir):
    ...
    if not valid_df.empty:
        ...
        ax.set_title(f'BAC vs Lambda - K={K} CJM with Mode Loss\n' +
                    f'(15 points, 512 corrected simulations, Mac Air M4 Optimized)',
    ...
    plt.tight_layout()
    plot_path = output_dir / 'bac_vs_lambda_corrected.png'
