#!/usr/bin/env python
"""
Step 2: Simulate from fitted HMM K=3 (Fast Mode) - Simple version without parallel
...
def simulate_hmm_K3(config_path='configs/daily_K3.yaml'):
    ...
    # Simulate sequences - Simple sequential approach
    logger.info(f"Generating {N_sim} simulated sequences (sequential)...")
    ...
    for i in tqdm(range(N_sim), desc="Simulating"):
        ...
        if (i + 1) % 50 == 0:
            logger.info(f"  Completed {i + 1}/{N_sim} simulations")
