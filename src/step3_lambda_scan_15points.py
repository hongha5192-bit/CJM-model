#!/usr/bin/env python
"""
Step 3: Lambda scan with 15 points optimized for Mac Air M4
Tests lambdas from 1e-2 to 1e5 using 512 simulations
"""
import numpy as np
import pandas as pd
import json
...
def main():
    """Run lambda scan with 15 points optimized for Mac Air M4"""

    logger.info("="*80)
    logger.info("LAMBDA SCAN WITH 15 POINTS (Mac Air M4 Optimized)")
    logger.info("="*80)
    ...
    # Lambda grid: 15 points from 1e-2 to 1e5
    # Balanced between MD file (29 points) and Mac Air M4 constraints
    lambda_grid = np.logspace(-2, 5, 15)
    ...
    # Load simulations
    sim_dir = Path('outputs/step2_sim/K3_512')
    ...
    batch_size = 3  # Process 3 lambdas at a time
    ...
    csv_path = output_dir / 'lambda_scores_15points.csv'
    df_results['K'] = K
...
def create_bac_plot(df_results, best_lambda, best_bac, K, output_dir):
    """Create BAC vs Lambda plot"""
    ...
    if not valid_df.empty:
        ...
        ax.set_title(f'BAC vs Lambda - K={K} CJM with Mode Loss\n' +
                    f'(15 points, 512 simulations, Mac Air M4 Optimized)',
    ...
    plt.tight_layout()
    plot_path = output_dir / 'bac_vs_lambda_15points.png'
